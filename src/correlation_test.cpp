...
void cor_calc(vector< vector<double> > datas, double *r)
{
  ...
  for(int i = 0; i<data_aves.size(); ++i)
    {
      cout<<"data_aves["<<i<<"]:"<<data_aves[i]<<endl;
    }

  double sigma1sigma2 = 0, sigma1sq = 0, sigma2sq = 0;
  for(int i=0; i<datas[0].size(); ++i)
    {
      cout << i <<", "<< datas[0][i]<<"," << datas[1][i]<<endl;

      double sigma1 = (double)datas[0][i]-(double)data_aves[0];
      double sigma2 = (double)datas[1][i]-(double)data_aves[1];
  ...
  cout << "s1s2:"<<sigma1sigma2<<", s1sq:"<<sigma1sq<<", s2sq:"<<sigma2sq<<endl;
  //cout<< "sqrt(s1sq):" << sqrt(sigma1sq) << ", sqrt(s2sq):" << sqrt(sigma2sq)<<endl;
  //cout << (double)sqrt(sigma1sq)*(double)sqrt(sigma2sq) << endl;
  //*r = (double)sigma1sigma2 / (double)(sqrt(sigma1sq)*sqrt(sigma2sq));  
  *r = (double)sigma1sigma2 / (double)sqrt(sigma1sq*sigma2sq);  
  //*r = (double)sigma1sigma2 / (double)(sigma1sq*sigma2sq);  
  //cout << *r << endl;
}
...
int main(int argc, char** argv)
{
  ...
  */
  ...
  for(int i=0 ;i<5; ++i)
    {
      data1.push_back(d1[i]);
      data2.push_back(d2[i]);
    }

  for(int i=0 ;i<data1.size(); ++i)
    {
      cout<<"data[1]:" << data1[i] << ", data[2]:"<<data2[i]<<endl;;
    }
