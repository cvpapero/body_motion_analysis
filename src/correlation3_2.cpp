  ...
  */
  ...
  for(int dt= -max_range; dt <= max_range; ++dt)
      ...
      if(fabs(corr) > threshold) 
	{
	  ...
	  printf("winsize:%d, (%2d, %2d) dt:(frame:%5d, time:%4.4f), r: %2.7f\n", 
	  	 winsize, i_index+1, j_index+1, dt, delay_time, corr);
	  ++ws_count[winsize];	  
	}      
...
int main(int argc, char** argv)
{
  ...
  for(int winsize = 3; winsize <= data[0][0].size(); ++winsize)
    {
      ws_count[winsize] = 0;
  ...
  map<int, int>::iterator it = ws_count.begin();

  while(it != ws_count.end())
    {
      cout<<"winsize:"<< it->first << ", count:" << it->second<<endl;
      ++it;
    }
